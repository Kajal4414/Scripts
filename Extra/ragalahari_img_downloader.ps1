...
if ($skippedCount -gt 0) { Write-Host " - " -NoNewline; Write-Host "Skipped: $skippedCount" -ForegroundColor Yellow -NoNewline }
if ($failedCount -gt 0) { Write-Host " - " -NoNewline; Write-Host "Failed: $failedCount" -ForegroundColor Red }
Write-Host "`nAll Images Downloaded At '$(Resolve-Path -Path $destinationFolder)'" -ForegroundColor Blue
